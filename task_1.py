...
def get_stat_com():
    top_data_df = pd.read_csv(TOP_1M_DOMAINS_FILE,  names = ['rank', 'domain'])
    top_data_10 = top_data_df[top_data_df['domain'].str.endswith('.com') ]
    top_data_10 = top_data_10.head(10)
    with open('top_data_10_com.csv', 'w') as f:
        f.write(top_data_10.to_csv(index=  False, header = False))
...
def print_data():
    with open('top_data_10.csv', 'r') as f:
        all_data = f.read()
    with open('top_data_10_com.csv', 'r') as f:
        all_data_com = f.read()
    print('Top domain in .RU')
    print(all_data)
    print('Top domain in .COM')
    print(all_data_com)
